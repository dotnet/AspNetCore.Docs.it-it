...
## <a name="component-classes"></a><span data-ttu-id="5dd6c-110">Classi di componenti</span><span class="sxs-lookup"><span data-stu-id="5dd6c-110">Component classes</span></span>
...
* <span data-ttu-id="5dd6c-138">`headingFontStyle` al valore della proprietà CSS per `font-style` .</span><span class="sxs-lookup"><span data-stu-id="5dd6c-138">`headingFontStyle` to the CSS property value for `font-style`.</span></span>
...
<span data-ttu-id="5dd6c-151">Per le cartelle personalizzate che contengono componenti, aggiungere una [`@using`][2] direttiva al componente padre o al file dell'app `_Imports.razor` .</span><span class="sxs-lookup"><span data-stu-id="5dd6c-151">For custom folders that hold components, add a [`@using`][2] directive to the parent component or to the app's `_Imports.razor` file.</span></span> <span data-ttu-id="5dd6c-152">L'esempio seguente rende disponibili i componenti nella `Components` cartella:</span><span class="sxs-lookup"><span data-stu-id="5dd6c-152">The following example makes components in the `Components` folder available:</span></span>
...
<span data-ttu-id="5dd6c-176">Il `Counter` componente può essere creato anche usando un file code-behind con una classe parziale:</span><span class="sxs-lookup"><span data-stu-id="5dd6c-176">The `Counter` component can also be created using a code-behind file with a partial class:</span></span>
...
### <a name="specify-a-base-class"></a><span data-ttu-id="5dd6c-182">Specificare una classe di base</span><span class="sxs-lookup"><span data-stu-id="5dd6c-182">Specify a base class</span></span>
...
<span data-ttu-id="5dd6c-191">Il markup seguente in `Pages/Index.razor` esegue il rendering di un' `HeadingComponent` istanza di:</span><span class="sxs-lookup"><span data-stu-id="5dd6c-191">The following markup in `Pages/Index.razor` renders a `HeadingComponent` instance:</span></span>
...
<span data-ttu-id="5dd6c-202">`Pages/RouteParameter.razor`:</span><span class="sxs-lookup"><span data-stu-id="5dd6c-202">`Pages/RouteParameter.razor`:</span></span>
...
* <span data-ttu-id="5dd6c-216">Per assegnare il campo, la proprietà o il metodo di un componente padre al parametro di un componente figlio, anteporre il simbolo al nome del campo, della proprietà o del metodo `@` .</span><span class="sxs-lookup"><span data-stu-id="5dd6c-216">To assign a parent component's field, property, or method to a child component's parameter, prefix the field, property, or method name with the `@` symbol.</span></span> <span data-ttu-id="5dd6c-217">Per assegnare il risultato di un' [espressione C# implicita](xref:mvc/views/razor#implicit-razor-expressions) a un parametro, anteporre l'espressione implicita a un `@` simbolo.</span><span class="sxs-lookup"><span data-stu-id="5dd6c-217">To assign the result of an [implicit C# expression](xref:mvc/views/razor#implicit-razor-expressions) to a parameter, prefix the implicit expression with an `@` symbol.</span></span>
  ...
  * <span data-ttu-id="5dd6c-219">Valore del `title` campo.</span><span class="sxs-lookup"><span data-stu-id="5dd6c-219">The value of the `title` field.</span></span>
...
* <span data-ttu-id="5dd6c-234">Per assegnare il risultato di un' [espressione C# esplicita](xref:mvc/views/razor#explicit-razor-expressions) nel componente padre al parametro di un componente figlio, racchiudere l'espressione tra parentesi con un `@` prefisso di simbolo.</span><span class="sxs-lookup"><span data-stu-id="5dd6c-234">To assign the result of an [explicit C# expression](xref:mvc/views/razor#explicit-razor-expressions) in the parent component to a child component's parameter, surround the expression in parentheses with an `@` symbol prefix.</span></span>
  ...
  > <span data-ttu-id="5dd6c-243">Gli attributi componente non supportano contenuto complesso (C# misto e markup).</span><span class="sxs-lookup"><span data-stu-id="5dd6c-243">Component attributes do not support complex content (mixed C# and markup).</span></span>
...
<span data-ttu-id="5dd6c-264">`Shared/ChildComponent.razor`:</span><span class="sxs-lookup"><span data-stu-id="5dd6c-264">`Shared/ChildComponent.razor`:</span></span>
...
* <span data-ttu-id="5dd6c-298">Aggiungere un [`@ref`][4] attributo al componente figlio.</span><span class="sxs-lookup"><span data-stu-id="5dd6c-298">Add an [`@ref`][4] attribute to the child component.</span></span>
...
### <a name="avoid-thread-blocking-calls"></a><span data-ttu-id="5dd6c-320">Evitare chiamate di blocco del thread</span><span class="sxs-lookup"><span data-stu-id="5dd6c-320">Avoid thread-blocking calls</span></span>
...
* <span data-ttu-id="5dd6c-327">In Blazor WebAssembly registrare il servizio come singleton in `Program.Main` :</span><span class="sxs-lookup"><span data-stu-id="5dd6c-327">In Blazor WebAssembly, register the service as singleton in `Program.Main`:</span></span>
...
### <a name="when-to-use-key"></a><span data-ttu-id="5dd6c-355">Quando usare la \@ chiave</span><span class="sxs-lookup"><span data-stu-id="5dd6c-355">When to use \@key</span></span>
...
### <a name="what-values-to-use-for-key"></a><span data-ttu-id="5dd6c-365">Valori da usare per la \@ chiave</span><span class="sxs-lookup"><span data-stu-id="5dd6c-365">What values to use for \@key</span></span>
...
* <span data-ttu-id="5dd6c-380">Il componente figlio viene sottoposto a rendering con uno o più valori di parametro dal componente padre.</span><span class="sxs-lookup"><span data-stu-id="5dd6c-380">The child component is rendered with one or more parameter values from the parent component.</span></span>
...
<span data-ttu-id="5dd6c-389">Il `Expander` componente viene aggiunto a un componente padre che può chiamare <xref:Microsoft.AspNetCore.Components.ComponentBase.StateHasChanged%2A> :</span><span class="sxs-lookup"><span data-stu-id="5dd6c-389">The `Expander` component is added to a parent component that may call <xref:Microsoft.AspNetCore.Components.ComponentBase.StateHasChanged%2A>:</span></span>
...
<span data-ttu-id="5dd6c-390">Inizialmente, i `Expander` componenti si comportano in modo indipendente quando le proprietà vengono disposte `Expanded` .</span><span class="sxs-lookup"><span data-stu-id="5dd6c-390">Initially, the `Expander` components behave independently when their `Expanded` properties are toggled.</span></span> <span data-ttu-id="5dd6c-391">I componenti figlio gestiscono gli Stati come previsto.</span><span class="sxs-lookup"><span data-stu-id="5dd6c-391">The child components maintain their states as expected.</span></span> <span data-ttu-id="5dd6c-392">Quando <xref:Microsoft.AspNetCore.Components.ComponentBase.StateHasChanged%2A> viene chiamato nell'elemento padre, il `Expanded` parametro del primo componente figlio viene reimpostato sul valore iniziale ( `true` ).</span><span class="sxs-lookup"><span data-stu-id="5dd6c-392">When <xref:Microsoft.AspNetCore.Components.ComponentBase.StateHasChanged%2A> is called in the parent, the `Expanded` parameter of the first child component is reset back to its initial value (`true`).</span></span> <span data-ttu-id="5dd6c-393">Il valore del secondo `Expander` componente `Expanded` non viene reimpostato perché nel secondo componente non viene eseguito il rendering del contenuto figlio.</span><span class="sxs-lookup"><span data-stu-id="5dd6c-393">The second `Expander` component's `Expanded` value isn't reset because no child content is rendered in the second component.</span></span>

<span data-ttu-id="5dd6c-394">Per mantenere lo stato nello scenario precedente, usare un *campo privato* nel `Expander` componente per mantenerne lo stato attivato/disattivato.</span><span class="sxs-lookup"><span data-stu-id="5dd6c-394">To maintain state in the preceding scenario, use a *private field* in the `Expander` component to maintain its toggled state.</span></span>

<span data-ttu-id="5dd6c-395">Il componente modificato seguente `Expander` :</span><span class="sxs-lookup"><span data-stu-id="5dd6c-395">The following revised `Expander` component:</span></span>

* <span data-ttu-id="5dd6c-396">Accetta il `Expanded` valore del parametro component dall'elemento padre.</span><span class="sxs-lookup"><span data-stu-id="5dd6c-396">Accepts the `Expanded` component parameter value from the parent.</span></span>
* <span data-ttu-id="5dd6c-397">Assegna il valore del parametro component a un *campo privato* ( `expanded` ) nell' [evento OnInitialized](xref:blazor/components/lifecycle#component-initialization-methods).</span><span class="sxs-lookup"><span data-stu-id="5dd6c-397">Assigns the component parameter value to a *private field* (`expanded`) in the [OnInitialized event](xref:blazor/components/lifecycle#component-initialization-methods).</span></span>
* <span data-ttu-id="5dd6c-398">Usa il campo privato per mantenere lo stato di attivazione/disattivo interno, che dimostra come evitare la scrittura diretta in un parametro.</span><span class="sxs-lookup"><span data-stu-id="5dd6c-398">Uses the private field to maintain its internal toggle state, which demonstrates how to avoid writing directly to a parameter.</span></span>
...
<span data-ttu-id="5dd6c-399">Per ulteriori informazioni, vedere [ Blazor errore di associazione bidirezionale (DotNet/aspnetcore #24599)](https://github.com/dotnet/aspnetcore/issues/24599).</span><span class="sxs-lookup"><span data-stu-id="5dd6c-399">For additional information, see [Blazor Two Way Binding Error (dotnet/aspnetcore #24599)](https://github.com/dotnet/aspnetcore/issues/24599).</span></span> 

## <a name="apply-an-attribute"></a><span data-ttu-id="5dd6c-400">Applicare un attributo</span><span class="sxs-lookup"><span data-stu-id="5dd6c-400">Apply an attribute</span></span>

<span data-ttu-id="5dd6c-401">Gli attributi possono essere applicati ai Razor componenti con la [`@attribute`][7] direttiva.</span><span class="sxs-lookup"><span data-stu-id="5dd6c-401">Attributes can be applied to Razor components with the [`@attribute`][7] directive.</span></span> <span data-ttu-id="5dd6c-402">Nell'esempio seguente viene applicato l' [ `[Authorize]` attributo](xref:Microsoft.AspNetCore.Authorization.AuthorizeAttribute) alla classe Component:</span><span class="sxs-lookup"><span data-stu-id="5dd6c-402">The following example applies the [`[Authorize]` attribute](xref:Microsoft.AspNetCore.Authorization.AuthorizeAttribute) to the component class:</span></span>
...
## <a name="conditional-html-element-attributes"></a><span data-ttu-id="5dd6c-403">Attributi dell'elemento HTML condizionale</span><span class="sxs-lookup"><span data-stu-id="5dd6c-403">Conditional HTML element attributes</span></span>

<span data-ttu-id="5dd6c-404">Gli attributi degli elementi HTML vengono sottoposti a rendering in modo condizionale in base al valore .NET.</span><span class="sxs-lookup"><span data-stu-id="5dd6c-404">HTML element attributes are conditionally rendered based on the .NET value.</span></span> <span data-ttu-id="5dd6c-405">Se il valore è `false` o `null` , l'attributo non viene sottoposto a rendering.</span><span class="sxs-lookup"><span data-stu-id="5dd6c-405">If the value is `false` or `null`, the attribute isn't rendered.</span></span> <span data-ttu-id="5dd6c-406">Se il valore è `true` , l'attributo viene sottoposto a rendering ridotto a icona.</span><span class="sxs-lookup"><span data-stu-id="5dd6c-406">If the value is `true`, the attribute is rendered minimized.</span></span>

<span data-ttu-id="5dd6c-407">Nell'esempio seguente, `IsCompleted` determina se `checked` viene eseguito il rendering nel markup dell'elemento:</span><span class="sxs-lookup"><span data-stu-id="5dd6c-407">In the following example, `IsCompleted` determines if `checked` is rendered in the element's markup:</span></span>
...
<span data-ttu-id="5dd6c-408">Se `IsCompleted` è `true` , viene eseguito il rendering della casella di controllo come segue:</span><span class="sxs-lookup"><span data-stu-id="5dd6c-408">If `IsCompleted` is `true`, the check box is rendered as:</span></span>

```html
<input type="checkbox" checked />
```

<span data-ttu-id="5dd6c-409">Se `IsCompleted` è `false` , viene eseguito il rendering della casella di controllo come segue:</span><span class="sxs-lookup"><span data-stu-id="5dd6c-409">If `IsCompleted` is `false`, the check box is rendered as:</span></span>

```html
<input type="checkbox" />
```

<span data-ttu-id="5dd6c-410">Per ulteriori informazioni, vedere [ Razor riferimento alla sintassi per ASP.NET Core](xref:mvc/views/razor).</span><span class="sxs-lookup"><span data-stu-id="5dd6c-410">For more information, see [Razor syntax reference for ASP.NET Core](xref:mvc/views/razor).</span></span>

> [!WARNING]
> <span data-ttu-id="5dd6c-411">Alcuni attributi HTML, ad esempio [`aria-pressed`](https://developer.mozilla.org/docs/Web/Accessibility/ARIA/Roles/button_role#Toggle_buttons) , non funzionano correttamente quando il tipo .NET è un `bool` .</span><span class="sxs-lookup"><span data-stu-id="5dd6c-411">Some HTML attributes, such as [`aria-pressed`](https://developer.mozilla.org/docs/Web/Accessibility/ARIA/Roles/button_role#Toggle_buttons), don't function properly when the .NET type is a `bool`.</span></span> <span data-ttu-id="5dd6c-412">In questi casi, usare un `string` tipo anziché un `bool` .</span><span class="sxs-lookup"><span data-stu-id="5dd6c-412">In those cases, use a `string` type instead of a `bool`.</span></span>

## <a name="raw-html"></a><span data-ttu-id="5dd6c-413">HTML non elaborato</span><span class="sxs-lookup"><span data-stu-id="5dd6c-413">Raw HTML</span></span>

<span data-ttu-id="5dd6c-414">Le stringhe vengono in genere sottoposte a rendering usando i nodi di testo DOM, il che significa che qualsiasi markup che può contenere viene ignorato e considerato come testo letterale.</span><span class="sxs-lookup"><span data-stu-id="5dd6c-414">Strings are normally rendered using DOM text nodes, which means that any markup they may contain is ignored and treated as literal text.</span></span> <span data-ttu-id="5dd6c-415">Per eseguire il rendering del codice HTML non elaborato, eseguire il wrapping del contenuto HTML in un `MarkupString` valore.</span><span class="sxs-lookup"><span data-stu-id="5dd6c-415">To render raw HTML, wrap the HTML content in a `MarkupString` value.</span></span> <span data-ttu-id="5dd6c-416">Il valore viene analizzato in formato HTML o SVG e inserito nel DOM.</span><span class="sxs-lookup"><span data-stu-id="5dd6c-416">The value is parsed as HTML or SVG and inserted into the DOM.</span></span>

> [!WARNING]
> <span data-ttu-id="5dd6c-417">Il rendering di codice HTML non elaborato costruito da un'origine non attendibile costituisce un rischio per la **sicurezza** e deve essere evitato.</span><span class="sxs-lookup"><span data-stu-id="5dd6c-417">Rendering raw HTML constructed from any untrusted source is a **security risk** and should be avoided!</span></span>

<span data-ttu-id="5dd6c-418">Nell'esempio seguente viene illustrato l'utilizzo del `MarkupString` tipo per aggiungere un blocco di contenuto HTML statico all'output sottoposto a rendering di un componente:</span><span class="sxs-lookup"><span data-stu-id="5dd6c-418">The following example shows using the `MarkupString` type to add a block of static HTML content to the rendered output of a component:</span></span>
...
## <a name="razor-templates"></a><span data-ttu-id="5dd6c-419">Razor modelli</span><span class="sxs-lookup"><span data-stu-id="5dd6c-419">Razor templates</span></span>

<span data-ttu-id="5dd6c-420">I frammenti di rendering possono essere definiti usando la Razor sintassi del modello.</span><span class="sxs-lookup"><span data-stu-id="5dd6c-420">Render fragments can be defined using Razor template syntax.</span></span> <span data-ttu-id="5dd6c-421">Razor i modelli sono un modo per definire un frammento di interfaccia utente e presupporre il formato seguente:</span><span class="sxs-lookup"><span data-stu-id="5dd6c-421">Razor templates are a way to define a UI snippet and assume the following format:</span></span>

```razor
@<{HTML tag}>...</{HTML tag}>
```

<span data-ttu-id="5dd6c-422">Nell'esempio seguente viene illustrato come specificare <xref:Microsoft.AspNetCore.Components.RenderFragment> <xref:Microsoft.AspNetCore.Components.RenderFragment%601> i valori e ed eseguire il rendering dei modelli direttamente in un componente.</span><span class="sxs-lookup"><span data-stu-id="5dd6c-422">The following example illustrates how to specify <xref:Microsoft.AspNetCore.Components.RenderFragment> and <xref:Microsoft.AspNetCore.Components.RenderFragment%601> values and render templates directly in a component.</span></span> <span data-ttu-id="5dd6c-423">I frammenti di rendering possono anche essere passati come argomenti ai [componenti basati su modelli](xref:blazor/components/templated-components).</span><span class="sxs-lookup"><span data-stu-id="5dd6c-423">Render fragments can also be passed as arguments to [templated components](xref:blazor/components/templated-components).</span></span>
...
<span data-ttu-id="5dd6c-424">Output del codice precedente sottoposto a rendering:</span><span class="sxs-lookup"><span data-stu-id="5dd6c-424">Rendered output of the preceding code:</span></span>
...
## <a name="static-assets"></a><span data-ttu-id="5dd6c-425">Asset statici</span><span class="sxs-lookup"><span data-stu-id="5dd6c-425">Static assets</span></span>

<span data-ttu-id="5dd6c-426">Blazorsegue la convenzione di ASP.NET Core app che collocano asset statici nella [ `web root (wwwroot)` cartella](xref:fundamentals/index#web-root)del progetto.</span><span class="sxs-lookup"><span data-stu-id="5dd6c-426">Blazor follows the convention of ASP.NET Core apps placing static assets under the project's [`web root (wwwroot)` folder](xref:fundamentals/index#web-root).</span></span>

<span data-ttu-id="5dd6c-427">Usare un percorso relativo di base ( `/` ) per fare riferimento alla radice Web per un asset statico.</span><span class="sxs-lookup"><span data-stu-id="5dd6c-427">Use a base-relative path (`/`) to refer to the web root for a static asset.</span></span> <span data-ttu-id="5dd6c-428">Nell'esempio seguente, `logo.png` si trova fisicamente nella `{PROJECT ROOT}/wwwroot/images` cartella:</span><span class="sxs-lookup"><span data-stu-id="5dd6c-428">In the following example, `logo.png` is physically located in the `{PROJECT ROOT}/wwwroot/images` folder:</span></span>

```razor
<img alt="Company logo" src="/images/logo.png" />
```

<span data-ttu-id="5dd6c-429">Razor i componenti **non** supportano la notazione tilde-barra ( `~/` ).</span><span class="sxs-lookup"><span data-stu-id="5dd6c-429">Razor components do **not** support tilde-slash notation (`~/`).</span></span>

<span data-ttu-id="5dd6c-430">Per informazioni sull'impostazione del percorso di base di un'app, vedere <xref:blazor/host-and-deploy/index#app-base-path> .</span><span class="sxs-lookup"><span data-stu-id="5dd6c-430">For information on setting an app's base path, see <xref:blazor/host-and-deploy/index#app-base-path>.</span></span>

## <a name="tag-helpers-arent-supported-in-components"></a><span data-ttu-id="5dd6c-431">Gli helper tag non sono supportati nei componenti</span><span class="sxs-lookup"><span data-stu-id="5dd6c-431">Tag Helpers aren't supported in components</span></span>

<span data-ttu-id="5dd6c-432">[`Tag Helpers`](xref:mvc/views/tag-helpers/intro) non sono supportati in Razor componenti ( `.razor` file).</span><span class="sxs-lookup"><span data-stu-id="5dd6c-432">[`Tag Helpers`](xref:mvc/views/tag-helpers/intro) aren't supported in Razor components (`.razor` files).</span></span> <span data-ttu-id="5dd6c-433">Per fornire funzionalità di tipo Helper tag in Blazor , creare un componente con la stessa funzionalità dell'helper tag e utilizzare il componente.</span><span class="sxs-lookup"><span data-stu-id="5dd6c-433">To provide Tag Helper-like functionality in Blazor, create a component with the same functionality as the Tag Helper and use the component instead.</span></span>

## <a name="scalable-vector-graphics-svg-images"></a><span data-ttu-id="5dd6c-434">Immagini SVG (Scalable Vector Graphics)</span><span class="sxs-lookup"><span data-stu-id="5dd6c-434">Scalable Vector Graphics (SVG) images</span></span>

<span data-ttu-id="5dd6c-435">Poiché Blazor esegue il rendering di HTML, le immagini supportate dal browser, incluse le immagini SVG (Scalable Vector Graphics) `.svg` , sono supportate tramite il `<img>` Tag:</span><span class="sxs-lookup"><span data-stu-id="5dd6c-435">Since Blazor renders HTML, browser-supported images, including Scalable Vector Graphics (SVG) images (`.svg`), are supported via the `<img>` tag:</span></span>

```html
<img alt="Example image" src="some-image.svg" />
```

<span data-ttu-id="5dd6c-436">Analogamente, le immagini SVG sono supportate nelle regole CSS di un file di foglio di stile ( `.css` ):</span><span class="sxs-lookup"><span data-stu-id="5dd6c-436">Similarly, SVG images are supported in the CSS rules of a stylesheet file (`.css`):</span></span>
...
<span data-ttu-id="5dd6c-437">Tuttavia, il markup SVG inline non è supportato in tutti gli scenari.</span><span class="sxs-lookup"><span data-stu-id="5dd6c-437">However, inline SVG markup isn't supported in all scenarios.</span></span> <span data-ttu-id="5dd6c-438">Se si inserisce un `<svg>` tag direttamente in un file di componente ( `.razor` ), il rendering delle immagini di base è supportato, ma molti scenari avanzati non sono ancora supportati.</span><span class="sxs-lookup"><span data-stu-id="5dd6c-438">If you place an `<svg>` tag directly into a component file (`.razor`), basic image rendering is supported but many advanced scenarios aren't yet supported.</span></span> <span data-ttu-id="5dd6c-439">Ad esempio, i `<use>` tag non sono attualmente rispettati e [`@bind`][10] non possono essere usati con alcuni tag svg.</span><span class="sxs-lookup"><span data-stu-id="5dd6c-439">For example, `<use>` tags aren't currently respected, and [`@bind`][10] can't be used with some SVG tags.</span></span> <span data-ttu-id="5dd6c-440">Per ulteriori informazioni, vedere [supporto per SVG in Blazor (#18271 DotNet/aspnetcore)](https://github.com/dotnet/aspnetcore/issues/18271).</span><span class="sxs-lookup"><span data-stu-id="5dd6c-440">For more information, see [SVG support in Blazor (dotnet/aspnetcore #18271)](https://github.com/dotnet/aspnetcore/issues/18271).</span></span>

## <a name="whitespace-rendering-behavior"></a><span data-ttu-id="5dd6c-441">Comportamento di rendering degli spazi vuoti</span><span class="sxs-lookup"><span data-stu-id="5dd6c-441">Whitespace rendering behavior</span></span>

::: moniker range=">= aspnetcore-5.0"

<span data-ttu-id="5dd6c-442">[`@preservewhitespace`](xref:mvc/views/razor#preservewhitespace)Se la direttiva non viene utilizzata con un valore di `true` , per impostazione predefinita viene rimosso uno spazio vuoto aggiuntivo se:</span><span class="sxs-lookup"><span data-stu-id="5dd6c-442">Unless the [`@preservewhitespace`](xref:mvc/views/razor#preservewhitespace) directive is used with a value of `true`, extra whitespace is removed by default if:</span></span>

* <span data-ttu-id="5dd6c-443">Iniziali o finali all'interno di un elemento.</span><span class="sxs-lookup"><span data-stu-id="5dd6c-443">Leading or trailing within an element.</span></span>
* <span data-ttu-id="5dd6c-444">Iniziali o finali in un `RenderFragment` parametro.</span><span class="sxs-lookup"><span data-stu-id="5dd6c-444">Leading or trailing within a `RenderFragment` parameter.</span></span> <span data-ttu-id="5dd6c-445">Ad esempio, il contenuto figlio viene passato a un altro componente.</span><span class="sxs-lookup"><span data-stu-id="5dd6c-445">For example, child content passed to another component.</span></span>
* <span data-ttu-id="5dd6c-446">Precede o segue un blocco di codice C#, ad esempio `@if` o `@foreach` .</span><span class="sxs-lookup"><span data-stu-id="5dd6c-446">It precedes or follows a C# code block, such as `@if` or `@foreach`.</span></span>

<span data-ttu-id="5dd6c-447">La rimozione degli spazi vuoti può influire sull'output sottoposto a rendering quando si usa una regola CSS, ad esempio `white-space: pre` .</span><span class="sxs-lookup"><span data-stu-id="5dd6c-447">Whitespace removal might affect the rendered output when using a CSS rule, such as `white-space: pre`.</span></span> <span data-ttu-id="5dd6c-448">Per disabilitare questa ottimizzazione delle prestazioni e mantenere lo spazio vuoto, effettuare una delle operazioni seguenti:</span><span class="sxs-lookup"><span data-stu-id="5dd6c-448">To disable this performance optimization and preserve the whitespace, take one of the following actions:</span></span>

* <span data-ttu-id="5dd6c-449">Aggiungere la `@preservewhitespace true` direttiva all'inizio del `.razor` file per applicare la preferenza a un componente specifico.</span><span class="sxs-lookup"><span data-stu-id="5dd6c-449">Add the `@preservewhitespace true` directive at the top of the `.razor` file to apply the preference to a specific component.</span></span>
* <span data-ttu-id="5dd6c-450">Aggiungere la `@preservewhitespace true` direttiva all'interno `_Imports.razor` di un file per applicare la preferenza a un'intera sottodirectory o all'intero progetto.</span><span class="sxs-lookup"><span data-stu-id="5dd6c-450">Add the `@preservewhitespace true` directive inside an `_Imports.razor` file to apply the preference to an entire subdirectory or the entire project.</span></span>

<span data-ttu-id="5dd6c-451">Nella maggior parte dei casi, non è necessaria alcuna azione, perché le app in genere continuano a funzionare normalmente (ma più velocemente).</span><span class="sxs-lookup"><span data-stu-id="5dd6c-451">In most cases, no action is required, as apps typically continue to behave normally (but faster).</span></span> <span data-ttu-id="5dd6c-452">Se la rimozione dello spazio vuoto causa problemi per un particolare componente, utilizzare `@preservewhitespace true` in tale componente per disabilitare questa ottimizzazione.</span><span class="sxs-lookup"><span data-stu-id="5dd6c-452">If stripping whitespace causes any problem for a particular component, use `@preservewhitespace true` in that component to disable this optimization.</span></span>

::: moniker-end

::: moniker range="< aspnetcore-5.0"

<span data-ttu-id="5dd6c-453">Lo spazio vuoto viene mantenuto nel codice sorgente di un componente.</span><span class="sxs-lookup"><span data-stu-id="5dd6c-453">Whitespace is retained in a component's source code.</span></span> <span data-ttu-id="5dd6c-454">Viene eseguito il rendering del testo con solo spazi vuoti nell'Document Object Model del browser (DOM) anche quando non è presente alcun effetto visivo.</span><span class="sxs-lookup"><span data-stu-id="5dd6c-454">Whitespace-only text renders in the browser's Document Object Model (DOM) even when there's no visual effect.</span></span>

<span data-ttu-id="5dd6c-455">Si consideri il Razor codice componente seguente:</span><span class="sxs-lookup"><span data-stu-id="5dd6c-455">Consider the following Razor component code:</span></span>
...
<span data-ttu-id="5dd6c-456">Nell'esempio precedente viene eseguito il rendering degli spazi vuoti non necessari seguenti:</span><span class="sxs-lookup"><span data-stu-id="5dd6c-456">The preceding example renders the following unnecessary whitespace:</span></span>

* <span data-ttu-id="5dd6c-457">All'esterno del `@foreach` blocco di codice.</span><span class="sxs-lookup"><span data-stu-id="5dd6c-457">Outside of the `@foreach` code block.</span></span>
* <span data-ttu-id="5dd6c-458">Intorno all' `<li>` elemento.</span><span class="sxs-lookup"><span data-stu-id="5dd6c-458">Around the `<li>` element.</span></span>
* <span data-ttu-id="5dd6c-459">Intorno all' `@item.Text` output.</span><span class="sxs-lookup"><span data-stu-id="5dd6c-459">Around the `@item.Text` output.</span></span>

<span data-ttu-id="5dd6c-460">Un elenco contenente 100 elementi restituisce 402 aree di spazio vuoto e nessuno degli spazi vuoti influiscono visivamente sull'output sottoposto a rendering.</span><span class="sxs-lookup"><span data-stu-id="5dd6c-460">A list containing 100 items results in 402 areas of whitespace, and none of the extra whitespace visually affects the rendered output.</span></span>

<span data-ttu-id="5dd6c-461">Quando si esegue il rendering HTML statico per i componenti, gli spazi vuoti all'interno di un tag non vengono conservati</span><span class="sxs-lookup"><span data-stu-id="5dd6c-461">When rendering static HTML for components, whitespace inside a tag isn't preserved.</span></span> <span data-ttu-id="5dd6c-462">Ad esempio, visualizzare l'origine del componente seguente nell'output di cui è stato eseguito il rendering:</span><span class="sxs-lookup"><span data-stu-id="5dd6c-462">For example, view the source of the following component in rendered output:</span></span>

```razor
<img     alt="My image"   src="img.png"     />
```

<span data-ttu-id="5dd6c-463">Lo spazio vuoto non è mantenuto dal Razor markup precedente:</span><span class="sxs-lookup"><span data-stu-id="5dd6c-463">Whitespace isn't preserved from the preceding Razor markup:</span></span>
...
## <a name="additional-resources"></a><span data-ttu-id="5dd6c-464">Risorse aggiuntive</span><span class="sxs-lookup"><span data-stu-id="5dd6c-464">Additional resources</span></span>

* <span data-ttu-id="5dd6c-465"><xref:blazor/security/server/threat-mitigation>: Include informazioni aggiuntive sulla creazione di Blazor Server app che devono essere confrontate con l'esaurimento delle risorse.</span><span class="sxs-lookup"><span data-stu-id="5dd6c-465"><xref:blazor/security/server/threat-mitigation>: Includes guidance on building Blazor Server apps that must contend with resource exhaustion.</span></span>
